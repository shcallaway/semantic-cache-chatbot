...
class PineconeVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        print(f"Available indexes before initialization: {[idx.name for idx in indexes]}")
            ...
            if hasattr(stats, 'total_vector_count'):
                ...
                if isinstance(stats, dict):
                    if 'total_vector_count' in stats:
                        ...
                        total = sum(ns.get('vector_count', 0) for ns in stats['namespaces'].values())
            ...
            try:
                ...
                if self.config.ttl_days is None or self.config.ttl_days <= 0:
                    ...
                    try:
                        ...
                        print(f"Attempting to delete namespace: {self.config.namespace}")
                        self.index.delete(
                            namespace=self.config.namespace,
                            delete_all=True
                        )
                        ...
                        try:
                            ...
                            for i in range(0, len(vector_ids), batch_size):
                                ...
                                print(f"Deleting batch {i//batch_size + 1} of {(len(vector_ids)-1)//batch_size + 1}...")
                                self.index.delete(
                                    ids=batch,
                                    namespace=self.config.namespace
                                )
                    ...
                    if old_vector_ids:
                        ...
                        try:
                            ...
                            for i in range(0, len(old_vector_ids), batch_size):
                                ...
                                print(f"Deleting batch {i//batch_size + 1} of {(len(old_vector_ids)-1)//batch_size + 1}...")
                                self.index.delete(
                                    ids=batch,
                                    namespace=self.config.namespace
                                )
